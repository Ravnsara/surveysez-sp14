...
{# check to see if we have a valid SurveyID
	...
	if($mySurvey->isValid)
	{ #check to see if we have a valid SurveyID
		echo "Survey No: " . $mySurvey->SurveyID . "<br />";
		echo "Survey Title: <b>" . $mySurvey->Title . "</b><br />";
		echo "Survey Description: " . $mySurvey->Description . "<br /><br />";
...
{//will create a list of responses for this survey
	...
	{#records exist - process
	    ...
	    echo '<div align="center">There are ' . $myPager->showTotal() . ' ' . $itemz . '.</div>';
		while($row = mysqli_fetch_assoc($result))
		{# process each row
	         echo '<div align="center"><a href="' . VIRTUAL_PATH . 'surveys/response_view.php?id=' . (int)$row['ResponseID'] . '">' . dbOut($row['DateAdded']) . '</a><br />';
	        //echo 'Created By: ' . dbOut($row['AdminName']) . ' | On: ' . dbOut($row['DateAdded']) . '</div><br />';
		}
